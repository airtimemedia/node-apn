    ...
    return this.getStream().then( stream => {
      ...
      return new Promise ( resolve => {
        stream.on("end", () => {
          if (status === "200") {
            ...
            if (status === "403" && response.reason === "ExpiredProviderToken" && retryCount < 2) {
              this.config.logger.warn({ notification, device }, 'XXX expired token, retrying');
            ...
            this.config.logger.warn({ error, notification, device }, 'XXX stream end error event, failing');
            resolve({ device, error });
          }
        });

        stream.on("unprocessed", () => {
          this.config.logger.warn({ notification, device }, 'XXX unprocessed stream, retrying');
        ...
        stream.on("error", err => {
          ...
          this.config.logger.warn({ error, notification, device }, 'XXX stream error event, failing');
          resolve({ device, error });
        });
      ...
      this.config.logger.warn({ error, notification, device }, 'XXX catch error event, failing');
      return { device, error };
    });
  };
