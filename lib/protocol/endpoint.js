...
module.exports = function(dependencies) {
  ...
  Endpoint.prototype._setupHTTP2Pipeline = function _setupHTTP2Pipeline() {
    const serializer = new protocol.Serializer(this.logger.child("serializer"));
    const compressor = new protocol.Compressor(this.logger.child("compressor"), "REQUEST");
    const deserializer = new protocol.Deserializer(this.logger.child("deserializer"));
    const decompressor = new protocol.Decompressor(this.logger.child("decompressor"), "RESPONSE");
  ...
  Endpoint.prototype._connect = function connect() {
    ...
    this._connection = new protocol.Connection(this.logger, 1);
  ...
  Endpoint.prototype._goaway = function goaway(frame) {
    this.logger.warn({ frame }, 'XXX endpoint _goaway');
  ...
  Endpoint.prototype._close = function close() {
    this.logger.warn({ streamIds: this._connection._streamIds}, 'XXX endpoint _close');
    ...
    this._connection._streamIds.forEach( (stream, id) => {
      ...
      if (this.lastStream < id) {
        this.logger.warn({ id }, 'XXX endpoint _close unprocessed');
        stream.emit("unprocessed");

      } else if (this.lastError) {
        this.logger.warn({ id }, 'XXX endpoint _close error');
